...
Each tenant has a unique API_SECRET for device integrations and API access.

#### Tenant-Specific API_SECRET (Recommended)

Each tenant receives a unique API_SECRET during registration. This secret can be used in plain text or as a SHA-1 hash:

```http
# Header method with plain text
GET /api/v1/entries
api-secret: your-plain-api-secret

# Header method with SHA-1 hash
GET /api/v1/entries
api-secret: your-hashed-api-secret

# Query parameter method with plain text
GET /api/v1/entries?secret=your-plain-api-secret

# Query parameter method with SHA-1 hash
GET /api/v1/entries?secret=your-hashed-api-secret
```

#### Generating SHA-1 Hash

If you prefer to use the hashed version:
```javascript
...
#### Master API_SECRET (System Level)

For system administrators, a master API_SECRET may be configured that works across all tenants. This follows the same authentication pattern as tenant-specific secrets.

#### Important Notes

- Each tenant's API_SECRET only grants access to their own data
- New tenants automatically receive an API_SECRET during registration
- Existing tenants can have API_SECRETs generated by system administrators
- Both plain text and SHA-1 hash formats are accepted
- The API_SECRET is ideal for device integrations (CGM uploaders, pumps, etc.)
