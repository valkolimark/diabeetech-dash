  ...
  const manager = {
    ...
    async initializeTenant(tenant) {
      ...
      try {
        ...
        // Get tenant settings from master database
        const masterCtx = {
          store: ctx.store,  // This should be the master database
          masterDb: ctx.store.db
        };
        const tenantSettings = require('../models/tenant-settings')(env, masterCtx);
