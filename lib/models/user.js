...
function init(env, ctx) {
  ...
  api.hashPassword = async function(password) {
    return await bcrypt.hash(password, 10);
  };

  // Verify password
  api.verifyPassword = async function(password, hash) {
  ...
  api.create = async function(userData) {
    ...
    const user = {
      ...
      passwordHash: await api.hashPassword(userData.password),
  ...
  api.authenticate = async function(tenantId, email, password) {
    ...
    if (!await api.verifyPassword(password, user.passwordHash)) {
      return null;
    }
