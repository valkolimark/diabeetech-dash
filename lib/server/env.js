...
function setMultiTenant () {
  env.MULTI_TENANT_ENABLED = readENVTruthy('MULTI_TENANT_ENABLED', false);
  if (env.MULTI_TENANT_ENABLED) {
    env.MASTER_MONGODB_URI = readENV('MASTER_MONGODB_URI');
    env.JWT_SECRET = readENV('JWT_SECRET');
    env.BASE_DOMAIN = readENV('BASE_DOMAIN');
    env.EMAIL_HOST = readENV('EMAIL_HOST');
    env.EMAIL_PORT = readENV('EMAIL_PORT', 587);
    env.EMAIL_SECURE = readENVTruthy('EMAIL_SECURE', false);
    env.EMAIL_USER = readENV('EMAIL_USER');
    env.EMAIL_PASS = readENV('EMAIL_PASS');
    env.EMAIL_FROM = readENV('EMAIL_FROM', 'noreply@nightscout.com');
    
    // Validate required multi-tenant settings
    if (!env.MASTER_MONGODB_URI) {
      env.err.push({ desc: 'MASTER_MONGODB_URI is required when MULTI_TENANT_ENABLED is true' });
    }
    if (!env.JWT_SECRET || env.JWT_SECRET.length < 32) {
      env.err.push({ desc: 'JWT_SECRET is required and should be at least 32 characters when MULTI_TENANT_ENABLED is true' });
    }
    if (!env.BASE_DOMAIN) {
      env.err.push({ desc: 'BASE_DOMAIN is required when MULTI_TENANT_ENABLED is true' });
    }
  }
}
