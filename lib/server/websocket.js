...
function init (env, ctx, server) {
  ...
  function start () {
    ...
    // Trigger initial data load if not done yet after a short delay
    setTimeout(function() {
      if (!initialDataLoaded) {
        console.log(LOG_WS + 'Triggering initial data load');
      ...
      socket.on('authorize', function authorize (message, callback) {
        ...
        verifyAuthorization(message, remoteIP, function verified (err, authorization) {
          ...
          if (socketAuthorization.read) {
            ...
            if (initialDataLoaded && lastData && lastData.dataWithRecentStatuses) {
              ...
              // Initial data not loaded yet, send minimal data with profiles from ctx.ddata
              console.log(LOG_WS + 'Initial data not loaded, sending data from ctx.ddata');
              
              // Use ctx.ddata directly which should have profiles loaded
              if (ctx.ddata && ctx.ddata.dataWithRecentStatuses) {
                let data = ctx.ddata.dataWithRecentStatuses();
                ...
                // Fallback: construct minimal data structure
                let data = {
                  sgvs: ctx.ddata.sgvs || [],
                  treatments: ctx.ddata.treatments || [],
                  profiles: ctx.ddata.profiles || [],
                  mbgs: ctx.ddata.mbgs || [],
                  cals: ctx.ddata.cals || [],
                  devicestatus: ctx.ddata.devicestatus || [],
                  food: ctx.ddata.food || [],
                  dbstats: ctx.ddata.dbstats || {}
                };
                ...
                console.log(LOG_WS + 'Sending fallback data structure with profiles:', data.profiles.length > 0 ? 'available' : 'empty');
                socket.emit('dataUpdate', data);
              }
            }
          }
  ...
  function update () {
    ...
    if (!initialDataLoaded && ctx.ddata.profiles && ctx.ddata.profiles.length > 0) {
      initialDataLoaded = true;
      console.log(LOG_WS + 'Initial data loaded, profiles available');
    }
  };
