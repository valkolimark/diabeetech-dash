...
function create (env, ctx) {
  ...
  app.use('/static/admin', express.static(path.join(__dirname, '../../static/admin'), { maxAge }));
  ...
  app.get('/admin*', (req, res, next) => {
    ...
    try {
      ...
      if (!features.adminDashboard || !features.adminDashboard.enabled) {
        return next(); // Pass to next handler if not enabled
      }
      ...
      // If features module doesn't exist or error, continue to next handler
      next();
    }
  });
