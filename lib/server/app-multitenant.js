...
function create (env, ctx) {
  ...
  // Admin static files must be served before tenant resolver
  app.use('/static/admin', express.static(resolvePath('/static/admin'), { maxAge }));
  ...
  // JSON parsing error handler - must be before routes that use express.json()
  ...
  app.get('/register', (req, res) => {
    const { getCaptchaConfig } = require('../middleware/captcha');
    const captchaConfig = getCaptchaConfig(env);
    // Pass all variables at the root level for EJS
  ...
  // Test route for debugging captcha
  app.get('/register-test', (req, res) => {
    const { getCaptchaConfig } = require('../middleware/captcha');
    const captchaConfig = getCaptchaConfig(env);
    res.render('register-test.html', { 
      locals: { ...app.locals, captchaConfig },
      captchaConfig: captchaConfig
    });
  });
  ...
  // TEST ROUTE - to verify routing works before tenant resolver
  app.get('/test-admin-route', (req, res) => {
    res.json({ 
      message: 'Test route works - before tenant resolver',
      timestamp: new Date().toISOString()
    });
  });

  // Admin routes - MUST be before tenant resolver to bypass tenant requirements
  
  // Admin dashboard route (superadmin only, no tenant required)
  // CRITICAL: This MUST be before tenant resolver to work properly
  app.get('/admin', (req, res) => {
    console.log('Admin route hit - exact match');
    // Check if admin dashboard feature is enabled
    try {
      const features = require('../../config/features');
      if (!features.adminDashboard || !features.adminDashboard.enabled) {
        return res.status(404).send('Admin dashboard not enabled');
      }
      ...
      // If features module doesn't exist or error
      return res.status(404).send('Admin dashboard not available');
    }
  });
  
  app.get('/admin/*', (req, res) => {
    console.log('Admin route hit - wildcard match');
    // Check if admin dashboard feature is enabled
    try {
      const features = require('../../config/features');
      if (!features.adminDashboard || !features.adminDashboard.enabled) {
      ...
      // Serve the admin dashboard HTML for any admin subroute
      const adminPath = path.join(__dirname, '../../views/admin-dashboard.html');
      res.sendFile(adminPath);
    } catch (err) {
      // If features module doesn't exist or error
  ...
  // Set context on app for admin API to access
  app.set('ctx', ctx);

  // Admin API routes (superadmin only, no tenant required)
  app.use('/api/v1/admin', bodyParser.json({ limit: 1048576 * 50 }), (req, res, next) => {
    try {
      const features = require('../../config/features');
      if (!features.adminDashboard || !features.adminDashboard.enabled) {
        return res.status(404).json({ error: 'Admin API not enabled' });
      }
      // Admin routes handle their own auth, bypass tenant requirements
      const adminRouter = require('../api/admin');
      adminRouter(req, res, next);
    } catch (err) {
      return res.status(404).json({ error: 'Admin API not available' });
    }
  });
  ...
  Object.keys(appPages).forEach(function(page) {
    app.get(page, tenantResolver, (req, res) => {
      var renderPage = appPages[page];
      var locals = app.locals;
      
      // Check if this is admin page and tenant doesn't have admin access
      if (renderPage.type === 'admin' && (!req.tenant || !req.tenant.isAdmin)) {
        return res.status(403).json({
          status: 403,
          message: 'Access denied',
          error: 'Admin tools not available for this tenant'
        });
      }
  ...
  // Middleware to check if tenant has admin access
  ...
  app.use(function(err, req, res, next) {
    // Don't log JSON parsing errors as they're already handled
