...
function create (env, ctx) {
  ...
  app.use((req, res, next) => {
    ...
    const publicPaths = ['/api/v1/status', '/api/v1/echo', '/manifest.json', '/sw.js'];
    if (publicPaths.some(path => req.path.startsWith(path))) {
      return next();
    }

    // For API routes, use JWT auth
    if (req.path.startsWith('/api/')) {
      return auth.authenticate(req, res, next);
    }

    // For web pages, redirect to login if not authenticated
  ...
  Object.keys(appPages).forEach(function(page) {
    app.get(page, auth.authenticate, (req, res) => {
  ...
  // API routes
  app.use('/api-docs', express.static(resolvePath('/views/swagger-ui-dist'), { maxAge }));
  app.use('/swagger-ui-dist', (req, res) => {
    res.redirect(307, '/api-docs');
  });

  app.use('/api/v1', bodyParser.json({ limit: 1048576 * 50 }), createTenantAwareApiMiddleware(apiRoot));
