...
function init (env, ctx, server) {
  ...
  function listeners () {
    io.sockets.on('connection', async function(socket) {
      ...
      const authInfo = await authenticateSocket(socket);
      if (!authInfo) {
        return;  // Socket was disconnected in authenticateSocket
      }

      const { user, tenant, tenantDb } = authInfo;
      ...
      const tenantCtx = Object.create(ctx);
      tenantCtx.store = {
        db: tenantDb,
