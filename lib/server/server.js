...
// Load appropriate modules based on multi-tenant configuration
const bootevent = env.MULTI_TENANT_ENABLED ? require('./bootevent-multitenant') : require('./bootevent');
const createApp = env.MULTI_TENANT_ENABLED ? require('./app-multitenant') : require('./app');

bootevent(env, language).boot(function booted (ctx) {

    console.log('Boot event processing completed');
    console.log('Multi-tenant mode:', env.MULTI_TENANT_ENABLED ? 'ENABLED' : 'DISABLED');
    
    var app = createApp(env, ctx);
    ...
    ///////////////////////////////////////////////////
    var websocketModule = env.MULTI_TENANT_ENABLED ? require('./websocket-multitenant') : require('./websocket');
    var websocket = websocketModule(env, ctx, server);

    //after startup if there are no alarms send all clear
    let sendStartupAllClearTimer = setTimeout(function sendStartupAllClear () {
      // Skip startup notifications in multi-tenant mode since notifications are per-tenant
      if (env.MULTI_TENANT_ENABLED) {
        console.log('Multi-tenant mode: Skipping global startup notification');
        return;
      }
      
      if (ctx.notifications && typeof ctx.notifications.findHighestAlarm === 'function') {
