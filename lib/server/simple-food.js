...
function simpleFood() {
  ...
  // Note: body parsing should be handled by the parent app
  // If needed, we can add express.json() middleware here
  food.use(express.json());

  // Main food editor page - server rendered
  food.get('/', async function(req, res) {
    console.log('Food editor accessed');
    console.log('Tenant:', req.tenant ? req.tenant.subdomain : 'no tenant');
    console.log('Context exists:', !!req.ctx);
    console.log('Store exists:', !!(req.ctx && req.ctx.store));
    console.log('DB exists:', !!(req.ctx && req.ctx.store && req.ctx.store.db));
    
    try {
      // Check if ctx exists
      if (!req.ctx || !req.ctx.store || !req.ctx.store.db) {
        console.error('Food editor: Missing context or database connection');
        console.error('req.ctx:', JSON.stringify(req.ctx));
        console.error('req.tenant:', JSON.stringify(req.tenant));
  ...
  food.post('/api/create', async function(req, res) {
    console.log('Food create endpoint called');
