    ...
    try {
      ...
      if (!req.ctx.plugins) {
        ...
        if (tenantSettings && (!tenantSettings.enable || tenantSettings.enable.length === 0)) {
          ...
          tenantSettings.enable = ['bgnow', 'rawbg', 'direction', 'timeago', 'dbsize', 'delta', 'ar2', 'clock'];
        }
        
        // Ensure clock is in the enabled plugins if not already there
        ...
        console.log('Tenant settings enable list after ensuring clock:', tenantSettings.enable);
        ...
        // Create a context object for plugins that includes settings and extended settings
        const pluginCtx = Object.assign({}, ctx, {
          settings: tenantSettings,
          extendedSettings: tenantSettings.extendedSettings || {},
          language: req.ctx.language || ctx.language,
          levels: req.ctx.levels,
          moment: req.ctx.moment
        });
        
        // Clear the module cache to ensure fresh plugins instance for each tenant
        delete require.cache[require.resolve('../plugins')];
        delete require.cache[require.resolve('../plugins/index.js')];
        
        const pluginsModule = require('../plugins')(pluginCtx);
        console.log('Before registerServerDefaults, pluginCtx.settings.enable:', pluginCtx.settings.enable);
        pluginsModule.registerServerDefaults();
        console.log('After registerServerDefaults, enabled plugins:', pluginsModule.enabledPluginNames ? pluginsModule.enabledPluginNames() : 'No enabledPluginNames method');
      ...
      const tenantEnv = Object.assign({}, env, {
        ...
        extendedSettings: (req.ctx.settings && req.ctx.settings.extendedSettings) || {}
      });
      ...
      // Debug: Check if sandbox methods are available
      console.log('sbx.lastSGVMills available:', typeof sbx.lastSGVMills);
      console.log('sbx.lastSGVEntry available:', typeof sbx.lastSGVEntry);
      console.log('sbx.lastEntry available:', typeof sbx.lastEntry);
