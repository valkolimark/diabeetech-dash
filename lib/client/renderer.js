      ...
      sel.attr('cx', function(d) {
        ...
        .attr('opacity', function(d) {
          if (d.noFade) {
            ...
            // For current and recent data, use full opacity
            // Only fade future predictions
            if (!client.latestSGV) {
              return 1;
            }
            const timeDiff = d.mills - client.latestSGV.mills;
            // If this entry is before or at the latest SGV time, full opacity
            if (timeDiff <= 0) {
              return 1;
            }
            // Only apply fading to future predictions
