...
client.query = function query () {
  ...
  console.log('Clock client querying:', src);
  $.ajax(src, {
    error: function gotError (err) {
      console.error('Clock data fetch error:', err);
    }
    , success: function gotData (data) {
      console.log('Clock data received:', data);
...
client.render = function render () {
  ...
  if (!latestProperties.bgnow || !latestProperties.bgnow.sgvs) {
    console.error('BG data not available', latestProperties);
    return;
  }
  
  // Ensure settings are available
  if (!client.settings) {
    console.error('Clock settings not initialized');
    return;
  }
  ...
  // Backward compatible
  if (face === 'clock-color') {
    face = 'c' + (client.settings.showClockLastTime ? 'y' : 'n') + '13-sg35-' + (client.settings.showClockDelta ? 'dt14-' : '') + 'nl-ar25-nl-ag6';
  } else if (face === 'clock') {
    face = 'bn0-sg40';
  } else if (face === 'bgclock') {
    face = 'b' + (client.settings.showClockLastTime ? 'y' : 'n') + '13-sg35-' + (client.settings.showClockDelta ? 'dt14-' : '') + 'nl-ar25-nl-ag6';
  } else if (face === 'config') {
