  ...
  */
  ...
  function dataUpdate (received, headless) {
    ...
    if (received.profiles) {
      ...
      console.log('Raw profiles count:', received.profiles.length);
      console.log('Raw profiles:', JSON.stringify(received.profiles, null, 2));
      
      // Fix profile data structure if needed
      const fixedProfiles = received.profiles.map(p => {
        if (!p || !p.store || typeof p.store !== 'object') {
          console.log('Fixing profile structure for:', p);
          return {
            _id: p._id || 'defaultProfile',
            defaultProfile: p.defaultProfile || 'Default',
            startDate: p.startDate || new Date().toISOString(),
            mills: p.mills || Date.now(),
            units: p.units || 'mg/dl',
            store: p.store || {
              'Default': {
                dia: p.dia || 4,
                timezone: p.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone,
                basal: p.basal || [{ time: '00:00', value: 1.0 }],
                carbratio: p.carbratio || [{ time: '00:00', value: 10 }],
                sens: p.sens || [{ time: '00:00', value: 50 }],
                target_low: p.target_low || [{ time: '00:00', value: 80 }],
                target_high: p.target_high || [{ time: '00:00', value: 120 }],
                carbs_hr: p.carbs_hr || 20,
                delay: p.delay || 20,
                units: p.units || 'mg/dl'
              }
            }
      ...
      console.log('Fixed profiles:', fixedProfiles);
      profile.loadData(fixedProfiles);
      
      // Debug profile state after loading
      console.log('=== PROFILE STATE AFTER LOADING ===');
      console.log('Profile has data:', profile.hasData());
      ...
      try {
        ...
        const profileFromTime = profile.profileFromTime(currentTime);
        console.log('Profile from time:', profileFromTime);
        
        const activeProfile = profile.activeProfileToTime(currentTime);
        console.log('Active profile name:', activeProfile);
        
        const currentProfile = profile.getCurrentProfile(currentTime);
        console.log('Current profile data:', currentProfile);
      } catch (e) {
        console.error('Error checking profile functions:', e);
