...
client.init = function init (callback) {
  ...
  $.ajax({
    ...
    console.log('Failed to load server settings:', jqXHR);
    ...
    if (!jqXHR.readyState) {
      ...
      $('#loadingMessageText').html('Connecting to Diabeetech server failed, retrying every 5 seconds');
      window.setTimeout(window.Nightscout.client.init(), 5000);
      return;
    }

    // If 404 or 401, continue with default settings (multi-tenant mode)
    if (jqXHR.status === 404 || jqXHR.status === 401) {
      console.log('Status endpoint returned', jqXHR.status, ', continuing with default settings...');
...
client.load = function load (serverSettings, callback) {
  ...
  client.ctx = {
    data: client.ddata  // Connect ctx.data to ddata from the start
  ...
  // CRITICAL: Make profile available in ddata/ctx.data from the start
  ...
  client.sbx = sandbox.clientInit(client.ctx, client.now, client.ddata);
  ...
  // CRITICAL: Ensure ddata always has profile reference
  client.ddata.profile = profile;
  ...
  console.log('Attempting to connect socket with options:', socketOptions);
  client.socket = socket = io.connect(socketOptions);
  ...
  console.log('dataUpdate listener registered');
  ...
  client.authorizeSocket = function authorizeSocket () {
    ...
    // Get JWT token from localStorage for multi-tenant auth
    ...
    var auth_data = {
      ...
      , token: jwtToken || (client.authorized && client.authorized.token)
      , history: history
    };
    
    console.log('Sending authorize with data:', auth_data);
    console.log('Has token in auth_data:', !!auth_data.token);
    console.log('Has secret in auth_data:', !!auth_data.secret);

    socket.emit(
      'authorize'
      , auth_data
      , function authCallback (data) {
        console.log('Authorization callback received:', data);
        if (!data) {
          console.log('Crashed!');
          client.crashed();
          return;
        ...
        if (data.success || (data.read || hasRequiredPermission())) {
          // Successfully authorized, now subscribe for data
          console.log('Authorization successful, subscribing to data');
          ...
          // Authorization failed, request authentication
          console.log('Authorization failed, requesting authentication');
          client.hashauth.requestAuthentication(function afterRequest () {
  ...
  */
  ...
  function dataUpdate (received, headless) {
    ...
    console.log('dataUpdate received:', received);
    console.log('received data keys:', received ? Object.keys(received) : 'no data');
    if (received && received.sgvs) {
      console.log('SGV data count:', received.sgvs.length);
      if (received.sgvs.length > 0) {
        console.log('First SGV:', received.sgvs[0]);
        console.log('Last SGV:', received.sgvs[received.sgvs.length - 1]);
      }
    }
    if (received && received.treatments) {
      console.log('Treatment data count:', received.treatments.length);
    }

    // CRITICAL: Load profiles FIRST before any other processing
    if (received && received.profiles && received.profiles.length > 0) {
      console.log('=== LOADING PROFILES IMMEDIATELY ===');
      console.log('Profile count:', received.profiles.length);
      console.log('First profile:', JSON.stringify(received.profiles[0], null, 2));
      
      // Convert profiles to store structure if needed
      var convertedProfiles = received.profiles.map(function(p) {
        if (p && p.defaultProfile && !p.store) {
          console.log('Converting flat profile to store structure on client side');
          var converted = {
            defaultProfile: p.defaultProfile || 'Default',
            store: {}
          };
          converted.store[converted.defaultProfile] = p;
          return converted;
        }
        return p;
      });
      
      profile.loadData(convertedProfiles);
      console.log('Profile loaded, hasData:', profile.hasData());
      console.log('Profile data after loading:', profile.data);
    } else {
      console.warn('NO PROFILES RECEIVED IN DATA UPDATE');
      console.log('Received data keys:', received ? Object.keys(received) : 'no data');
    }
    ...
    // Store profiles before calling receiveDData
    var profiles = received.profiles;
    ...
    if (client.ddata.sgvs && client.ddata.sgvs.length > 0) {
      ...
      // Get the most recent SGV (highest mills value)
