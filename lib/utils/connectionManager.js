...
function init(env) {
  ...
  connectionManager.initMaster = async function() {
    ...
    try {
      ...
      const dbName = masterConnection.s.options.dbName || 'nightscout-master';
  ...
  connectionManager.getTenantDb = async function(tenant) {
    ...
    try {
      ...
      // Add stats method for database size information
      db.stats = function stats(callback) {
        // Use MongoDB's native dbStats command
        db.command({ dbStats: 1, scale: 1 })
          .then(function(stats) {
            console.log(`Database stats retrieved for ${dbName}:`, { dataSize: stats.dataSize, indexSize: stats.indexSize });
            callback(null, {
          ...
          .catch(function(err) {
            console.error(`Error getting database stats for ${dbName}:`, err);
            callback(err, null);
          });
      };
