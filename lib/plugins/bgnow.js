...
function init (ctx) {
  ...
  bgnow.setProperties = function setProperties (sbx) {
    console.log('bgnow.setProperties called');
    console.log('sbx.data.sgvs length:', sbx.data.sgvs ? sbx.data.sgvs.length : 'No SGVs');
    console.log('sbx.lastSGVMills function exists:', typeof sbx.lastSGVMills);
    console.log('sbx.lastSGVEntry function exists:', typeof sbx.lastSGVEntry);
    
    if (sbx.data.sgvs && sbx.data.sgvs.length > 0) {
      console.log('First SGV:', sbx.data.sgvs[0]);
      console.log('Last SGV:', sbx.data.sgvs[sbx.data.sgvs.length - 1]);
      if (sbx.lastSGVEntry) {
        var lastEntry = sbx.lastSGVEntry();
        console.log('Last SGV Entry from method:', lastEntry);
      }
    }
    ...
    console.log('Recent bucket:', recent ? 'Found' : 'Not found');
  ...
  bgnow.fillBuckets = function fillBuckets (sbx, opts) {
    ...
    console.log('fillBuckets - lastSGVMills:', lastSGVMills);
    
    if (!lastSGVMills) {
      console.log('fillBuckets - No lastSGVMills, returning empty buckets');
      return [];
    }
