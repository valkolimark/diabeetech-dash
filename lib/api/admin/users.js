  ...
  try {
    ...
    const [users, total] = await Promise.all([
      store.db.collection('users')
        ...
        .limit(limit)
        .toArray(),
      store.db.collection('users').countDocuments(filter)
    ]);
    
    // Get tenant names
    const tenantIds = [...new Set(users.map(u => u.tenant).filter(Boolean))];
    const tenants = await store.db.collection('tenants')
...
router.get('/:id', async function(req, res) {
  try {
    ...
    const user = await store.db.collection('users')
      .findOne({ _id: userId });
    ...
    let tenant = null;
    if (user.tenant) {
      tenant = await store.db.collection('tenants')
        .findOne({ _id: user.tenant });
    }
    
    // Get user activity stats
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const activityCount = await store.db.collection('admin_audit')
      .countDocuments({
        user: userId,
...
router.post('/', async function(req, res) {
  try {
    ...
    const existing = await store.db.collection('users')
    ...
    if (tenant) {
      const tenantExists = await store.db.collection('tenants')
        .findOne({ _id: tenant });
    ...
    await store.db.collection('users').insertOne(user);
...
router.put('/:id', async function(req, res) {
  try {
    ...
    if (tenant !== undefined) {
      // Validate tenant
      if (tenant && !(await store.db.collection('tenants').findOne({ _id: tenant }))) {
        return res.status(400).json({ 
          success: false, 
          error: 'Invalid tenant ID' 
        });
    ...
    const result = await store.db.collection('users')
      .updateOne(
        { _id: userId },
        { $set: update }
      );
...
router.delete('/:id', async function(req, res) {
  try {
    ...
    // Delete user
    await store.db.collection('users').deleteOne({ _id: userId });
  ...
  try {
    ...
    switch (action) {
      case 'delete':
        ...
        result = await store.db.collection('users')
      ...
      case 'change-tenant':
        if (data?.tenant && !(await store.db.collection('tenants').findOne({ _id: data.tenant }))) {
          return res.status(400).json({ 
            success: false, 
            error: 'Invalid tenant ID' 
          });
