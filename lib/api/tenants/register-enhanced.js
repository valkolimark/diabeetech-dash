...
function configure(env, ctx) {
  ...
  const { cloneSettingsFromReferenceTenant } = require('../../utils/cloneSettings');
  
  // Create middleware instances
  const captchaMiddleware = createCaptchaMiddleware(env);
  const rateLimiter = createRateLimiter(env);
  
  // Enhanced registration endpoint that includes Dexcom setup
  router.post('/', express.json(), rateLimiter, captchaMiddleware, async (req, res) => {
    try {
      const { 
      ...
      if (dexcom && dexcom.username && dexcom.password) {
        await tenantSettings.updateBridge(tenant.tenantId, {
          ...
          password: sanitizePassword(dexcom.password),
      ...
      // Clone settings from reference tenant (onepanman)
      const REFERENCE_TENANT = env.REFERENCE_TENANT || 'onepanman';
      await cloneSettingsFromReferenceTenant(env, ctx, REFERENCE_TENANT, tenantDb);
      
      // Override with user-specific settings if not cloned
      const profileCollection = tenantDb.collection(env.profile_collection || 'profile');
      const existingProfile = await profileCollection.findOne({ _id: 'defaultProfile' });
      
      if (!existingProfile) {
        // Create default Nightscout profile if cloning didn't provide one
        const profile = {
          _id: 'defaultProfile',
      ...
      } else if (units && units !== existingProfile.units) {
        // Update units if user selected different from reference
        await profileCollection.updateOne(
          { _id: 'defaultProfile' },
          { 
            $set: { 
              units: units,
              'store.Default.units': units 
            } 
          }
        );
      }
