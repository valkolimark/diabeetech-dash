  ...
  api.post('/simple', async function(req, res) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Total treatments in source: ${totalTreatments}`);
        
        // Get a sample treatment to check date field format
        const sampleTreatment = await treatmentsCollection.findOne({});
        if (sampleTreatment) {
          console.log('Sample treatment fields:', {
            created_at: sampleTreatment.created_at,
            created_at_type: typeof sampleTreatment.created_at,
            date: sampleTreatment.date,
            date_type: typeof sampleTreatment.date,
            timestamp: sampleTreatment.timestamp,
            timestamp_type: typeof sampleTreatment.timestamp
          });
        }
        ...
        // First try with created_at as ISO string
        treatments = await treatmentsCollection.find({
          created_at: { $gte: cutoffDate.toISOString() }
        }).limit(1000).toArray();
        
        // If no results, try with created_at as timestamp
        if (treatments.length === 0) {
          console.log('No treatments found with created_at ISO string, trying timestamp...');
          treatments = await treatmentsCollection.find({
        ...
        if (treatments.length === 0) {
          console.log('No treatments found with created_at, trying date field...');
          treatments = await treatmentsCollection.find({
            date: { $gte: cutoffDate.getTime() }
          }).limit(1000).toArray();
        }
        ...
        if (treatments.length === 0) {
          console.log('No treatments found with date filter, trying without filter...');
          treatments = await treatmentsCollection.find({}).limit(10).toArray();
          console.log('Sample treatments without filter:', treatments.slice(0, 2));
        }
