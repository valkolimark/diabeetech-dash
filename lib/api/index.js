...
function create (env, ctx) {
  ...
  app.disable('api');
  if (env.enclave && env.enclave.isApiKeySet()) {
    console.log('API KEY present, enabling API');
    app.enable('api');
  } else if (env.MULTI_TENANT_ENABLED !== 'false') {
    console.log('Multi-tenant mode, enabling API');
    app.enable('api');
  } else {
    console.log('API KEY has not been set, API disabled');
  }

  // Handle settings in multi-tenant mode
  ...
  if (settings.enable) {
    ...
    _each(settings.enable, function (enable) {
