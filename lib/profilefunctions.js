...
var moment = ctx.moment;
  ...
  profile.getCurrentProfile = function getCurrentProfile (time, spec_profile) {
    ...
    // Add safety checks to prevent errors when profile data is not loaded
    if (!data) {
      console.warn('Profile data not available yet');
      return profile.getDefaultProfile();
    }
    
    if (!data.store) {
      console.warn('Profile store not available, using default profile');
      return profile.getDefaultProfile();
    }
    
    if (!timeprofile || !data.store[timeprofile]) {
      console.warn('Profile', timeprofile, 'not found in store, using default profile');
      return profile.getDefaultProfile();
    }
    
    returnValue = data.store[timeprofile];
  ...
  profile.getDefaultProfile = function getDefaultProfile () {
    // Return a safe default profile to prevent errors
    return {
      dia: 4,
