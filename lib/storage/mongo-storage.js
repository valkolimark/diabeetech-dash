    ...
    if (mongo.db != null && !forceNewConnection) {
      ...
      const connect_with_retry = async function (i) {
        ...
        try {
          ...
          mongo.db.stats = function stats(callback) {
            // Use MongoDB's native dbStats command
            mongo.db.command({ dbStats: 1, scale: 1 })
              .then(function(stats) {
                console.log('Database stats retrieved:', { dataSize: stats.dataSize, indexSize: stats.indexSize });
                callback(null, {
