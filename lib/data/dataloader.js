...
const processForRuntime = (obj) => {
    Object.keys(obj).forEach(key => {
        if (typeof obj[key] === 'object' && obj[key]) {
          if (obj[key].hasOwnProperty('_id') && obj[key]._id != null) {
        ...
        const normalizeTreatments = (obj) => {
            Object.keys(obj).forEach(key => {
                if (typeof obj[key] === 'object' && obj[key]) {
                  ...
                  if (element.hasOwnProperty('_id') && element._id != null) {
...
function loadProfile(ddata, ctx, callback) {
    ctx.profile.last(function(err, results) {
        if (!err && results) {
            ...
            // Convert profile data to the expected structure
            var profilefunctions = require('../profilefunctions')(null, ctx);
            
            // Convert each profile to the store structure
            ddata.profiles = profiles.map(function(profile) {
                if (profile && profile.defaultProfile && !profile.store) {
                    console.log('Converting profile to store structure:', profile.defaultProfile);
                    return profilefunctions.convertToProfileStore(profile);
                }
                return profile;
            });
        }
